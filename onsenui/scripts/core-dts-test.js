...
const createTestScript = (elementDefinitions, objectDefinitions) => {
  ...
  code += `} from './onsenui';\n`;
  code += `\n`;
  // Guard the runtime named exports of the ESM entry. ngx-onsenui's bundle does
  // `import { PageLoader, _internal, notification, platform } from 'onsenui'`, so these
  // must stay type-importable; without this the d.ts test only exercised `import * as ons`.
  code += `import { PageLoader as _PageLoader, notification as _notification, platform as _platform, _internal as _onsInternal } from './onsenui';\n`;
  code += `void [_PageLoader, _notification, _platform, _onsInternal];\n`;
  code += `\n`;
